...
amass-src(){
    ...
    echo -e "\namass enum ${amass_options[@]} -d ${domain} -o ${tmp_dir}/amass_active_output.txt" >> "${log_execution_file}"
    echo "amass enum ${amass_options[@]} -passive -d ${domain} -o ${tmp_dir}/amass_passive_output.txt" >> "${log_execution_file}"
