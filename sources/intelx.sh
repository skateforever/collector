...
intelx-src(){
    ...
    echo -e "\ncurl ${curl_options[@]} -H \"User-agent: ${user_agent}\" -X POST \"https://2.intelx.io/phonebook/search?k=${intelx_api_key}\" -d '{\"term\":\"*.${domain}\",\"buckets\":[],\"lookuplevel\":0,\"maxresults\":100000,\"timeout\":0,\"datefrom\":\"\",\"dateto\":\"\",\"sort\":4,\"media\":0,\"terminate\":[],\"target\":1}'   " >> "${log_execution_file}"
    intelx_id="$(curl "${curl_options[@]}" \
        ...
        | jq -r '.id' 2>/dev/null)"
    sleep 2
    echo -e "\ncurl ${curl_options[@]} -H \"User-agent: ${user_agent}\" \"https://2.intelx.io/phonebook/result?k=${intelx_api_key}&id=${intelx_id}&limit=100000&offset=0\"" >> "${log_execution_file}"
    curl "${curl_options[@]}" \
        -H "User-agent: ${user_agent}" \
