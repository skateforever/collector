...
virustotal-src(){
    if [[ -n "${virustotal_api_url}" ]] && [[ -n "${virustotal_api_key}" ]]; then
        ...
        echo -e "\n$(redact_secrets "curl ${curl_options[@]} -H \"User-agent: ${user_agent}\" -H \"X-Apikey: ${virustotal_api_key}\" \"${virustotal_api_url}/${domain}/subdomains?limit=40\"")" >> "${log_execution_file}"
