...
diff_domains(){
    if [ -d "${report_dir}" ]; then 
        ...
        if [ -s "${report_dir}/domains_found.txt" ]; then
            ...
            echo -e "The message was: \n\tFile ${report_dir}/domains_alive.txt does not exist or is empty!" | notify -nc -silent -id "${notify_recon_channel}" > /dev/null 2>&1
        ...
        echo "The error occurred in the function diff_domains.sh!" | notify -nc -silent -id "${notify_recon_channel}" > /dev/null 2>&1
        echo -e "The message was: \n\tMake sure the directories structure was created. Stopping the script." | notify -nc -silent -id "${notify_recon_channel}" > /dev/null 2>&1
        message "${domain}" failed
        exit 1
    fi
}
