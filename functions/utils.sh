...
build_llm_prompt(){
    ...
    {
        echo "#########################################################################"
        echo "# COLLECTOR RECON BUNDLE — AUTOMATED LLM PENTEST DIRECTIVE"
        echo "#########################################################################"
        ...
        echo "#"
        echo "# ======================================================================"
        echo "# CRITICAL SYSTEM INSTRUCTIONS FOR THE LLM (EXECUTE IMMEDIATELY)"
        echo "# ======================================================================"
        echo "# ROLE: You are an elite, offensive-security AI assistant and Senior"
        echo "# Penetration Tester. Your job is to process this raw reconnaissance data"
        echo "# and immediately generate a tactical, highly actionable offensive report."
        echo "#"
        echo "# CORE INSTRUCTIONS:"
        echo "# 1. Data Correlation: Cross-reference findings across artifacts (e.g.,"
        echo "#    match alive subdomains with open nmap ports and nuclei vulnerabilities)."
        echo "# 2. No Generalities: Do not give generic advice. Provide exact commands"
        echo "#    (e.g., specific nmap scripts, ffuf paths, hydra syntax) tailored to"
        echo "#    the technologies discovered below."
        echo "# 3. Ground Truth: Strict adherence to data. Do not hallucinate hosts or vulnerabilities."
        echo "#"
        echo "# OUTPUT FORMAT REQUIREMENTS:"
        echo "# Respond immediately using a clean, well-structured Markdown document with"
        echo "# the following sections:"
        echo "#"
        echo "# ## 1. Executive Attack Surface Overview"
        echo "#    (A brief breakdown of total exposed infrastructure, technologies, and high-level risk)"
        echo "#"
        echo "# ## 2. High-Priority Actionable Targets (Top 3-5)"
        echo "#    (Pinpoint specific subdomains/IPs that look most promising or vulnerable)"
        echo "#"
        echo "# ## 3. Triage & Analysis of Automated Findings"
        echo "#    (Analyze findings from nuclei, nmap, and js_secrets/js_params. Flag suspicious tokens/sinks)"
        echo "#"
        echo "# ## 4. Tactical Attack Plan & Next Steps (THE MAIN SECTION)"
        echo "#    (Provide concrete manual testing methodologies, parameter fuzzing ideas,"
        echo "#     auth bypass checks, or SSRF vector probes. Intersperse with copy-pasteable commands)"
        echo "#"
        echo "# SANDBOX NOTE:"
        echo "# etc_hosts_file.txt is in ip<TAB>hostname format. To resolve vhosts in your"
        echo "# sandbox, append its contents to /etc/hosts before running any tooling."
        echo "#"
        echo "# ARTIFACTS IN THIS BUNDLE:"
        echo "# webapp_consolidated.txt — all live HTTP(S) URLs (DNS-resolved + validated vhosts)"
